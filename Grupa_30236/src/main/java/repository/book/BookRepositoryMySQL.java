...
//            return true;
    ...
    public boolean checkAvailability(Book book) {
        String sql = "SELECT COUNT(*) FROM book WHERE id = ? AND stock > 0";
    ...
    public void removeById(Long id) {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            ...
            if (rowsDeleted != 1) {
                // Poate să facăți și altceva, de exemplu aruncarea unei excepții sau gestionarea altfel a situației
                System.out.println("A apărut o problemă în timpul ștergerii cărții.");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public boolean updateBook(Long id, String newTitle, String newAuthor, LocalDate newPublishedDate) {
        String sql = "UPDATE book SET title = ?, author = ?, publishedDate = ? WHERE id = ?";

        try {
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1, newTitle);
            preparedStatement.setString(2, newAuthor);
            preparedStatement.setDate(3, Date.valueOf(newPublishedDate));
